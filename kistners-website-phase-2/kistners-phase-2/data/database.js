...
var arrangementsDB = new sqlite3.Database('./data/flowers.db');
...
/* For updating arrangments. */
arrangements.getAll = function (callback) {
  var sql = `SELECT * FROM arrangements`;
  arrangementsDB.all(sql, (err, rows) => {
    if (err) return callback(err);
    if (!rows) return callback(err);
...
arrangements.find = function(id, callback) {
  var sql = `SELECT arrangements.id, name, description,
              ...
              GROUP BY arrangements.id;`;
  arrangementsDB.get(sql, id, (err, row) => {
    // Check for errors
    if(err) return callback(err);
...
arrangements.create = function(arrangementData, callback) {
  ...
  arrangementsDB.run("INSERT INTO arrangements (name, description) VALUES (?, ?)",
    ...
    function(err) {
      ...
      arrangementsDB.run("INSERT INTO arrangements_photos (arrangement_id, filename, mime_type, data) VALUES (?, ?, ?, ?)",
...
arrangements.update = function(arrangementData, callback) {
  ...
  arrangementsDB.run("UPDATE arrangements SET name = ?, description = ? WHERE id = ?",
    arrangementData.name,
    arrangementData.description,
    arrangementData.id,
    function(err) {
...
arrangementImages.find = function(id, callback) {
  arrangementsDB.get("SELECT * FROM arrangements_photos WHERE id = ?;", id, (err, row) => {
