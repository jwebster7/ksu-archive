...
namespace Ksu.Cis300.Nim
{
    ...
    {
        /// <summary>
        /// Dictionary for holding instances of the board class
        /// </summary>
        private Dictionary<Board, Play> _correctMove = new Dictionary<Board, Play>();
        ...
        {
            Play p;
            if (_correctMove.TryGetValue(b, out p))
            {
                return p;
            }
            ...
            for (int i = 0; i < b.NumberOfPiles; i++)
            {
                for (int j = 1; j <= b.GetLimit(i); j++)
                {
                    ...
                    {
                        _correctMove.Add(b, p);
                        return p;
                    }
                }
            }
            _correctMove.Add(b, null);
            return null;
        }
